...
# Step Menu Execution Extension (Additive)

> This section is **additive only**.
> All rules above remain unchanged and take precedence.

------

## A. Step Menu Interaction Model (HARD)

### A.1 Mandatory Full Step Menu Output

For EVERY Cursor response:

```
PHASE=<INIT|OPTIMIZE|PACK>

STEP_MENU:
1) <STEP_ID> - <step purpose>
2) <STEP_ID> - <step purpose>
...

SELECT:
Choose exactly ONE STEP_ID to execute.
```

Cursor MUST NOT execute any step until a STEP_ID is selected.

------

## B. Phase Step Menus (Explicit, Selectable)

### INIT Phase (10 Steps)

```
INIT-0  Phase detection and phase lock confirmation
INIT-1  Run environment doctor
INIT-2  Repository hard-constraint gate check
INIT-3  Verify repo root and path safety
INIT-4  Scan and fix LFS pointer files
INIT-5  UI Kit integrity check (presence only)
INIT-6  app_icon resolution check (xml-only)
INIT-7  Fix Gradle sync blockers
INIT-8  Fix launch crash blockers
INIT-9  INIT completion criteria verification
```

### OPTIMIZE Phase (10 Steps)

```
OPTIMIZE-0  Confirm optimize goals and scope
OPTIMIZE-1  Performance tuning and allocation reduction
OPTIMIZE-2  Input responsiveness improvements
OPTIMIZE-3  UI clarity improvements (UI Kit only)
OPTIMIZE-4  HUD readability improvements
OPTIMIZE-5  Menu / pause / gameover refinement
OPTIMIZE-6  Gameplay feel tuning (Canvas-only effects)
OPTIMIZE-7  Visual feedback polish
OPTIMIZE-8  Regression safety check
OPTIMIZE-9  OPTIMIZE completion target verification
```

### PACK Phase (10 Steps)

```
PACK-0  Phase lock confirmation (PACK)
PACK-1  Run environment doctor
PACK-2  Run validation scripts (if present)
PACK-3  Build requested artifact (apk / zip)
PACK-4  UI and resource verification checklist
PACK-5  LFS and binary compliance check
PACK-6  Update registry (only if requested)
PACK-7  Git commit (only if requested)
PACK-8  Git push (only if requested)
PACK-9  Final PACK completion confirmation
```
...
- Cursor executes **only the selected step**
- No auto-advance
- If blocked, Cursor must explain why and reprint the menu

------

## Effective Ruleset Name

**CURSOR_RULES v2.0.2 + Step Menu Execution Extension**