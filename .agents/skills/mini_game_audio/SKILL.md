...
- Do not allow low-intelligence, auto, or speed-priority runs to skip `menu` and `play` BGM coverage
...
6. Synthesize fallback audio only when fetch is unavailable or insufficient.
7. Store every new asset in the shared library first.
8. Assign the needed assets into the target project.
9. Verify tracked `menu` and `play` BGM coverage before reporting success.
10. Report the audio brief, which roles were resolved, and which shared assets and project-local files were produced.
