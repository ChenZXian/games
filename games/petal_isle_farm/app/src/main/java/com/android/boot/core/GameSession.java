...
public class GameSession {
    ...
    public void start() {
        ...
        sessionLeft = 480f;
        combo = 0;
        estateBeauty = Math.max(estateBeauty, bestBeauty / 2);
    }
    ...
    public void harvest(CropPlot p) {
        ...
        estateBeauty += Math.max(1, beautyInt / 6);
    ...
    public String getZoneLabel() {
        if (progression.level >= 8) {
            return "Festival Plaza";
        }
        if (progression.level >= 6) {
            return "Glass Terrace";
        }
