...
public class MainActivity extends Activity {
    ...
    private void bindGame() {
        gameView.setListener(new GameView.GameListener() {
            @Override
            public void onHudChanged(int lives, int gold, int wave, int maxWave, int escaped, String status) {
                livesText.setText(String.format(Locale.US, "Lives %d", lives));
                goldText.setText(String.format(Locale.US, "Gold %d", gold));
                waveText.setText(String.format(Locale.US, "Wave %d/%d", wave, maxWave));
                statusText.setText(String.format(Locale.US, "%s  Escaped %d", status, escaped));
            }

            @Override
            public void onRunEnded(boolean cleared, int wave, int escaped, int stars, int lives) {
                audio.playSfx(cleared ? "win" : "fail");
                showResult(cleared, wave, escaped, stars, lives);
            }

            @Override
            public void onAudioEvent(String key) {
                audio.playSfx(key);
                if ("warning_horn".equals(key)) {
                    audio.playClimax();
                }
            }
        });
    ...
    private void bindButtons() {
        ...
        findViewById(R.id.archer_button).setOnClickListener(v -> selectBuild(GameView.BUILD_ARCHER));
        findViewById(R.id.barracks_button).setOnClickListener(v -> selectBuild(GameView.BUILD_BARRACKS));
        findViewById(R.id.cannon_button).setOnClickListener(v -> selectBuild(GameView.BUILD_CANNON));
        findViewById(R.id.mage_button).setOnClickListener(v -> selectBuild(GameView.BUILD_MAGE));
        findViewById(R.id.shrine_button).setOnClickListener(v -> selectBuild(GameView.BUILD_SHRINE));
        findViewById(R.id.hero_button).setOnClickListener(v -> commandHero());
        findViewById(R.id.reinforce_button).setOnClickListener(v -> useReinforce());
    ...
    private void commandHero() {
        audio.playSfx("ui_click");
        gameView.beginHeroCommand();
    }

    private void useReinforce() {
        audio.playSfx("ui_click");
        gameView.beginReinforceCommand();
    }

    private void useSpell() {
        audio.playSfx("ui_click");
        gameView.beginSpellCommand();
    }
    ...
    private void showResult(boolean cleared, int wave, int escaped, int stars, int lives) {
        ...
        topHud.setVisibility(View.VISIBLE);
        buildPanel.setVisibility(View.GONE);
        resultTitleText.setText(cleared ? "Bastion Held" : "Bastion Breached");
        resultStatsText.setText(String.format(Locale.US, "Wave %d  Lives %d  Escaped %d  Stars %d", wave, lives, escaped, stars));
    }
